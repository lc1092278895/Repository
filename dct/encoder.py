...
#cv2.imwrite('org.bmp', np.uint8(img))

# get size of the image
[h , w] = img.shape

imgf=img.astype('float')
dct_img = cv2.dft(imgf)
#log_dct_img=np.log(abs(dct_img))
reordered = zigzag(dct_img)
reshaped = np.reshape(reordered, (h, w))
cv2.imshow('dct_img', np.uint8(dct_img))
