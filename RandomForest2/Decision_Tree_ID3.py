...
def entropyCal(dfdata):
    ...
    for key in typeCount:
        p=float(typeCount[key])/dataSize ###某属性占比
...
def chooseBestFeatureToSplit(dfdata):
    ...
    numFeature = dfdata.shape[1]-1 #数据集属性个数---色泽、根蒂、敲声、纹理、脐部、触感等
    entropyBase = entropyCal(dfdata) #划分前样本集的信息熵---H（D）=  - ∑<1~typeCount> p(k) log2 p(k)
